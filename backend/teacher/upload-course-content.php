...
if ((isset($obj['course_id'], $obj['title'], $obj['content']) && !empty($obj['course_id']) && !empty($obj['title']) && !empty($obj['content'])) ||
    ...
    if (isset($obj['video_type']) && $obj['video_type'] === 'url') {
        ...
        $video_type = mysql_entities_fix_string($conn, $obj['video_type']);
        ...
        $path = 'uploads/';  // Specify the path to retrieve the uploaded files
        ...
        if (move_uploaded_file($file['tmp_name'], $target_file) && $file['error'] === UPLOAD_ERR_OK) {
            ...
            $query = "INSERT INTO course_content (course_id, title, content, video_type, video_url) VALUES ('$course_id', '$title', '$content', '$video_type', '$target')";
            $result = mysqli_query($conn, $query);
