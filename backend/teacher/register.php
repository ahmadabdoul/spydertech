...
if (empty($email) || empty($password) || empty($fullname) || empty($username) || empty($cellphone)) {
  ...
  $query = "SELECT * FROM teachers WHERE email = '$email' OR username = '$username'";
  ...
  if ($result && mysqli_num_rows($result) > 0) {
   ...
   $insertQuery = "INSERT INTO teachers (email, password, name, username, cellphone) VALUES ('$email', '$hashedPassword', '$fullname', '$username', '$cellphone')";
